...
internal class JobDriver_Teach : JobDriver
{
    ...
    protected override IEnumerable<Toil> MakeNewToils()
    {
        ...
        doTeaching.initAction = delegate
        {
            ...
            if(compMagic != null && compMagic.IsMagicUser && !student.story.traits.HasTrait(TorannMagicDefOf.Faceless))
            {
                ...
                if(compMight != null && compMight.IsMightUser)
                {
                    if(Rand.Chance(.5f))
                    {
                        isMageTeaching = false;
                        isFighterTeaching = true;
                    }
                }
            }
        ...
        doTeaching.tickAction = delegate
        {
            ...
            if((student.Position - pawn.Position).LengthHorizontal > 5)
            {
                success = false;
                age = duration + 1;
            }
        ...
        doTeaching.defaultDuration = duration;
        doTeaching.WithProgressBar(TargetIndex.B, delegate
        {
            ...
            return 1f - (float)doTeaching.actor.jobs.curDriver.ticksLeftThisToil / duration;
    ...
    private void DoTeachingEffects(Pawn student)
    {
        ...
        if(effectNum == 0)
        {
            for (int i = 0; i < rangeMax; i++)
            {
                ...
                TM_MoteMaker.ThrowGenericMote(mote, drawPos, pawn.Map, Rand.Range(.1f, .3f), 0.2f, .02f, .1f, 0, Rand.Range(8, 10), direction, direction);
        ...
        else if(effectNum ==1)
        {
            for (int i = 0; i < rangeMax / 2; i++)
            {
                ...
                TM_MoteMaker.ThrowGenericFleck(mote, rndPos, pawn.Map, Rand.Range(.6f, .9f), 0.4f, .1f, .8f, 0, Rand.Range(1, 1.5f), direction, direction);
                SoundInfo info = SoundInfo.InMap(new TargetInfo(pawn.Position, pawn.Map, false), MaintenanceType.None);
        ...
        else if(effectNum == 2)
        {
            for (int i = 0; i < rangeMax; i++)
            {
                ...
                TM_MoteMaker.ThrowGenericMote(mote, rndPos, pawn.Map, Rand.Range(1f, 1.4f), 0.7f, .2f, .9f, Rand.Range(0, 200), Rand.Range(.3f, .6f), direction, direction);
                SoundInfo info = SoundInfo.InMap(new TargetInfo(pawn.Position, pawn.Map, false), MaintenanceType.None);
        ...
        else if(effectNum == 3)
        {
            for (int i = 0; i < rangeMax; i++)
            {
                ...
                TM_MoteMaker.ThrowGenericMote(mote, rndPos, pawn.Map, Rand.Range(1f, 1.4f), 0.4f, Rand.Range(.1f, .4f), Rand.Range(.2f, .6f), Rand.Range(-100, 100), Rand.Range(1.2f, 2f), direction, direction);
                SoundInfo info = SoundInfo.InMap(new TargetInfo(pawn.Position, pawn.Map, false), MaintenanceType.None);
                info.pitchFactor = .8f;
                info.volumeFactor = 1.2f;
        ...
        else if (effectNum == 4)
        {
            for (int i = 0; i < rangeMax; i++)
            {
                ...
                TM_MoteMaker.ThrowGenericMote(mote, rndPos, pawn.Map, Rand.Range(.2f, .7f), Rand.Range(.2f, .6f), Rand.Range(0f, .8f), Rand.Range(.5f,.8f), Rand.Range(-50, 50), Rand.Range(.5f, 1f), direction, direction);
                SoundInfo info = SoundInfo.InMap(new TargetInfo(pawn.Position, pawn.Map, false), MaintenanceType.None);
    ...
    private void AssignMagicXP(Pawn student)
    {
        ...
        CompAbilityUserMagic mentorComp = pawn.GetCompAbilityUserMagic();

        if (studentComp != null && mentorComp != null)
        {
            try
            {
                int studentOpinion = student.relations.OpinionOf(pawn);
                int mentorOpinion = pawn.relations.OpinionOf(student);
                ...
                {
                    pawn.needs.joy.GainJoy(.4f, TorannMagicDefOf.Gaming_Cerebral);
                    pawn.needs.joy.GainJoy(.2f, TorannMagicDefOf.Social);
                }
    ...
    private void AssignMightXP(Pawn student)
    {
        ...
        CompAbilityUserMight mentorComp = pawn.GetCompAbilityUserMight();
        ...
        if (studentComp != null && mentorComp != null)
        {
            try
            {
                ...
                if (pawn.needs.joy != null)
                {
                    pawn.needs.joy.GainJoy(.4f, TorannMagicDefOf.Gaming_Dexterity);
                    pawn.needs.joy.GainJoy(.2f, TorannMagicDefOf.Social);
                }
                if (student.needs.joy != null)
                {
